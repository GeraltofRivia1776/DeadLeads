...
namespace EmeraldAI
{
    ...
    public class BulletProjectile : MonoBehaviour, IAvoidable
    {
        ...
        public void Initialize (GameObject owner, Transform currentTarget, BulletProjectileAbility abilityData)
        {
            ...
            StartTime = Time.time;
            CurrentAbilityData.BulletProjectileSettings.SpawnBulletEffect(Owner, transform.position, EmeraldComponent.CombatComponent.CurrentAttackTransform);
        }
        ...
        IEnumerator MoveProjectile()
        {
            ...
            while (!Collided)
            {
                ...
                if (CollisionCheckTimer >= CurrentAbilityData.BulletProjectileSettings.CollisionCheckSpeed)
                {
                    ...
                    //It has a forward offset of 0.5 units, which allows for reliable collision detection.
                    RaycastHit hit;
        ...
        void Impact (GameObject TargetHit, Vector3 HitPosition, Vector3 HitNormal)
        {
            ...
            Invoke(nameof(ImpactDespawn), 0.15f);
        }
